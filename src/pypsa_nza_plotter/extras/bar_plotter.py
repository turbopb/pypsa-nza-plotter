"""
Bar Chart Plotter - PlotLib v2.0

Creates bar charts (single, grouped, stacked, horizontal).

DESIGN PHILOSOPHY:
------------------
- Uses same SeriesConfig/PlotConfig as line plotter
- Returns (fig, ax) for consistency
- One function handles all bar chart types
- Simple, clear API

USAGE:
------
Single series:
    series = SeriesConfig(x=categories, y=values, label='Sales')
    fig, ax = create_bar_plot(series, config)

Multiple series (grouped):
    series1 = SeriesConfig(x=categories, y=values1, label='2023')
    series2 = SeriesConfig(x=categories, y=values2, label='2024')
    fig, ax = create_bar_plot([series1, series2], config, grouped=True)

Multiple series (stacked):
    fig, ax = create_bar_plot([series1, series2], config, stacked=True)

Horizontal bars:
    fig, ax = create_bar_plot(series, config, orientation='horizontal')
"""

import matplotlib
##matplotlib.use('Agg')  # Commented for interactive  # Commented for interactive
from matplotlib.figure import Figure
from matplotlib import pyplot as plt
import numpy as np
from typing import List, Union, Tuple, Optional
import sys
import os

# Add parent directory to path
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from models.plot_config import PlotConfig
from models.series_config import SeriesConfig


def create_bar_plot(
    series: Union[SeriesConfig, List[SeriesConfig]],
    config: PlotConfig,
    grouped: bool = False,
    stacked: bool = False,
    orientation: str = 'vertical',
    bar_width: float = 0.8
) -> Tuple[plt.Figure, plt.Axes]:
    """
    Create bar chart (single, grouped, or stacked).
    
    Args:
        series: SeriesConfig or list of SeriesConfig objects
        config: PlotConfig for global settings
        grouped: If True and multiple series, bars are grouped side-by-side
        stacked: If True and multiple series, bars are stacked
        orientation: 'vertical' or 'horizontal'
        bar_width: Width of bars (0-1, default 0.8)
    
    Returns:
        (fig, ax): Matplotlib figure and axes
    
    Examples:
        >>> # Single series
        >>> series = SeriesConfig(x=['Q1', 'Q2', 'Q3'], y=[100, 120, 115])
        >>> fig, ax = create_bar_plot(series, config)
        
        >>> # Grouped bars
        >>> series1 = SeriesConfig(x=['Q1', 'Q2'], y=[100, 120], label='2023')
        >>> series2 = SeriesConfig(x=['Q1', 'Q2'], y=[110, 125], label='2024')
        >>> fig, ax = create_bar_plot([series1, series2], config, grouped=True)
        
        >>> # Stacked bars
        >>> fig, ax = create_bar_plot([series1, series2], config, stacked=True)
        
        >>> # Horizontal bars
        >>> fig, ax = create_bar_plot(series, config, orientation='horizontal')
    """
    # Normalize to list
    if not isinstance(series, list):
        series = [series]
    
    # Validate
    if not series:
        raise ValueError("Must provide at least one series")
    
    if grouped and stacked:
        raise ValueError("Cannot be both grouped and stacked")
    
    # Create figure
    fig = plt.figure(
        figsize=(config.figure_width, config.figure_height),
        dpi=config.dpi,
        facecolor=config.figure_facecolor
    )
    
    ax = fig.add_subplot(111, facecolor=config.axes_facecolor)
    
    # Get categories from first series
    categories = series[0].x
    n_categories = len(categories)
    n_series = len(series)
    
    # Position calculations for grouped bars
    if grouped and n_series > 1:
        # Calculate bar width for each series
        individual_width = bar_width / n_series
        # Calculate positions for each series
        positions = []
        for i in range(n_series):
            offset = (i - n_series/2 + 0.5) * individual_width
            positions.append(np.arange(n_categories) + offset)
    else:
        # Single series or stacked - use same positions
        positions = [np.arange(n_categories)] * n_series
        individual_width = bar_width
    
    # Draw bars
    bottoms = np.zeros(n_categories) if stacked else None
    
    for i, s in enumerate(series):
        # Get bar properties from SeriesConfig
        color = s.color if s.color else None
        edgecolor = s.marker_edgecolor if s.marker_edgecolor else 'black'
        linewidth = s.marker_edgewidth if s.marker_edgewidth else 0
        hatch = s.hatch if s.hatch else None  # NEW: Hatch pattern
        alpha = 1.0  # Full opacity for bars by default
        
        if orientation == 'vertical':
            bars = ax.bar(
                positions[i],
                s.y,
                width=individual_width,
                bottom=bottoms if stacked else None,
                color=color,
                edgecolor=edgecolor,
                linewidth=linewidth,
                hatch=hatch,  # NEW: Apply hatch pattern
                alpha=alpha,
                label=s.label
            )
        else:  # horizontal
            bars = ax.barh(
                positions[i],
                s.y,
                height=individual_width,
                left=bottoms if stacked else None,
                color=color,
                edgecolor=edgecolor,
                linewidth=linewidth,
                hatch=hatch,  # NEW: Apply hatch pattern
                alpha=alpha,
                label=s.label
            )
        
        # Update bottom for stacking
        if stacked and bottoms is not None:
            bottoms = bottoms + np.array(s.y)
    
    # Set category labels
    if orientation == 'vertical':
        ax.set_xticks(np.arange(n_categories))
        ax.set_xticklabels(categories)
    else:
        ax.set_yticks(np.arange(n_categories))
        ax.set_yticklabels(categories)
    
    # Apply global formatting
    _apply_global_formatting(ax, config)
    
    return fig, ax


def _apply_global_formatting(ax: plt.Axes, config: PlotConfig) -> None:
    """
    Apply PlotConfig settings to axes (same as line plotter).
    
    This keeps formatting consistent across all plot types.
    """
    # Labels
    if config.x_label:
        ax.set_xlabel(
            config.x_label,
            fontsize=config.x_axis_label_size or config.axis_label_size,
            fontfamily=config.axis_label_family,
            fontweight=config.axis_label_weight,
            fontstyle=config.axis_label_style,
            color=config.axis_label_color
        )
    
    if config.y_label:
        ax.set_ylabel(
            config.y_label,
            fontsize=config.y_axis_label_size or config.axis_label_size,
            fontfamily=config.axis_label_family,
            fontweight=config.axis_label_weight,
            fontstyle=config.axis_label_style,
            color=config.axis_label_color
        )
    
    # Title
    if config.title:
        title_kwargs = {
            'fontsize': config.title_size,
            'fontfamily': config.title_family,
            'fontweight': config.title_weight,
            'fontstyle': config.title_style,
            'color': config.title_color
        }
        if config.title_pad is not None:
            title_kwargs['pad'] = config.title_pad
        ax.set_title(config.title, **title_kwargs)
    
    # Tick labels
    x_tick_size = config.x_tick_label_size or config.tick_label_size
    y_tick_size = config.y_tick_label_size or config.tick_label_size
    
    ax.tick_params(axis='x', labelsize=x_tick_size)
    ax.tick_params(axis='y', labelsize=y_tick_size)
    
    for label in ax.get_xticklabels():
        label.set_fontfamily(config.tick_label_family)
        label.set_fontweight(config.tick_label_weight)
        label.set_color(config.tick_label_color)
    
    for label in ax.get_yticklabels():
        label.set_fontfamily(config.tick_label_family)
        label.set_fontweight(config.tick_label_weight)
        label.set_color(config.tick_label_color)
    
    # Grid
    if config.show_grid:
        ax.grid(
            True,
            alpha=config.grid_alpha,
            linestyle=config.grid_style,
            linewidth=config.grid_linewidth,  # Fixed: was grid_width
            color=config.grid_color
        )
        ax.set_axisbelow(True)
    
    # Legend
    if config.show_legend:
        # Check if there are any labeled artists
        handles, labels = ax.get_legend_handles_labels()
        if handles and labels:
            legend = ax.legend(
                loc=config.legend_location,
                frameon=config.legend_frameon,
                framealpha=config.legend_framealpha,
                fontsize=config.legend_font_size
            )
            # Apply legend font weight if available
            if hasattr(config, 'legend_font_weight'):
                for text in legend.get_texts():
                    text.set_fontweight(config.legend_font_weight)
    
    # Spines
    if not config.show_top_spine:
        ax.spines['top'].set_visible(False)
    if not config.show_right_spine:
        ax.spines['right'].set_visible(False)
    if not config.show_left_spine:
        ax.spines['left'].set_visible(False)
    if not config.show_bottom_spine:
        ax.spines['bottom'].set_visible(False)
    
    # Tight layout
    if config.tight_layout:
        plt.tight_layout()


def save_plot(fig: plt.Figure, filename: str, dpi: int = 300) -> None:
    """
    Save figure to file.
    
    Args:
        fig: Matplotlib figure
        filename: Output filename
        dpi: Resolution (default: 300)
    """
    fig.savefig(filename, dpi=dpi, bbox_inches='tight')
    plt.close(fig)
